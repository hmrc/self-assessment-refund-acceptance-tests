...
//      case "ItsaViewerPage" => ItsaViewerPage.assertCurrentUrl()
  ...
  Then("""^the user sees the BARS error for (.*)$""") { (error: String) =>
    error match {
      case "an unsupported account" => findTextById("sortCode-error-summary") should be("You have entered a sort code which does not accept this type of payment. Check you have entered a valid sort code or enter details for a different account")
      case "failing validate checks" => findTextById("sortCode-error-summary") should be("Enter a valid combination of bank account number and sort code")
      case "failing verify checks" => findTextById("accountName-error-summary") should be("Enter the name on the account as it appears on bank statements. Do not copy and paste it")
      case "missing roll number" => findTextById("rollNumber-error-summary") should be("Building society roll number must be entered if you have one. It may also be called a reference code")
    }
  }
